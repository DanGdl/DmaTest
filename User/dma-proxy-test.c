...
const int TX_CHANNEL_COUNT = sizeof(tx_channel_names) / sizeof(*tx_channel_names);
const int RX_CHANNEL_COUNT = sizeof(tx_channel_names) / sizeof(*tx_channel_names);


/* Internal data which should work without tuning */
typedef struct Channel {
	ChannelBuffer_t* buf_ptr;
	int fd;
	pthread_t tid;
} Channel_t;

Channel_t tx_channels[TX_CHANNEL_COUNT];
Channel_t rx_channels[RX_CHANNEL_COUNT];
 ...
 */
void tx_thread(Channel_t* channel) {
	...
	for (buffer_id = 0; buffer_id < TX_BUFFER_COUNT; buffer_id += BUFFER_INCREMENT) {
		 ...
		 */
		channel->buf_ptr[buffer_id].length = test_size;

		if (verify) {
			for (i = 0; i < 1; i++) { // test_size / sizeof(unsigned int); i++)
				channel->buf_ptr[buffer_id].buffer[i] = i + in_progress_count;
			}
		}
		// Start the DMA transfer and this call is non-blocking
		ioctl(channel->fd, START_XFER, &buffer_id);
	...
	while (1) {
		 ...
		 */
		ioctl(channel->fd, FINISH_XFER, &buffer_id);
		if (channel->buf_ptr[buffer_id].status != PROXY_NO_ERROR) {
			printf("Proxy tx transfer error %d\n", channel->buf_ptr[buffer_id].status);
		}
		 ...
		 */
		if (verify) {
			unsigned int *buffer = (unsigned int *) &channel->buf_ptr[buffer_id].buffer;
...
void rx_thread(Channel_t* channel) {
...
int main(int argc, char *argv[]) {
	...
	for (i = 0; i < TX_CHANNEL_COUNT; i++) {
		...
		tx_channels[i].buf_ptr = (ChannelBuffer_t*) mmap(NULL, sizeof(ChannelBuffer_t) * TX_BUFFER_COUNT,
	...
	for (i = 0; i < RX_CHANNEL_COUNT; i++) {
		...
		rx_channels[i].buf_ptr = (ChannelBuffer_t*)mmap(NULL, sizeof(ChannelBuffer_t) * RX_BUFFER_COUNT,
	...
	for (i = 0; i < TX_CHANNEL_COUNT; i++) {
		...
		munmap(tx_channels[i].buf_ptr, sizeof(ChannelBuffer_t));
		close(tx_channels[i].fd);
	}
	for (i = 0; i < RX_CHANNEL_COUNT; i++) {
		munmap(rx_channels[i].buf_ptr, sizeof(ChannelBuffer_t));
