...
typedef enum ProxyStatus { PROXY_NO_ERROR = 0, PROXY_BUSY = 1, PROXY_TIMEOUT = 2, PROXY_ERROR = 3 } ProxyStatus_t;

typedef struct ChannelBuffer {
	unsigned int buffer[BUFFER_SIZE / sizeof(unsigned int)];
	ProxyStatus_t status;
	unsigned int length;
} ChannelBuffer_t __attribute__ ((aligned (1024)));		/* 64 byte alignment required for DMA, but 1024 handy for viewing memory */
